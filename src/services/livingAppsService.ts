...
import type { Unternehmensverwaltung, Schichteinteilung, Schichtartenverwaltung, Mitarbeiterverwaltung, CreateUnternehmensverwaltung, CreateSchichteinteilung, CreateSchichtartenverwaltung, CreateMitarbeiterverwaltung } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getUnternehmensverwaltung(): Promise<Unternehmensverwaltung[]> {
    ...
    })) as Unternehmensverwaltung[];
    return hydrateRecords(records, 'unternehmensverwaltung');
  }
  static async getUnternehmensverwaltungEntry(id: string): Promise<Unternehmensverwaltung | undefined> {
    ...
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Unternehmensverwaltung;
    return hydrateRecords([record], 'unternehmensverwaltung')[0];
  }
  static async createUnternehmensverwaltungEntry(fields: CreateUnternehmensverwaltung): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.UNTERNEHMENSVERWALTUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'unternehmensverwaltung') });
    return { ...data, record_id: data.id };
  }
  static async updateUnternehmensverwaltungEntry(id: string, fields: Partial<CreateUnternehmensverwaltung>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.UNTERNEHMENSVERWALTUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'unternehmensverwaltung') });
    return { ...data, record_id: data.id };
  }
  ...
  static async getSchichteinteilung(): Promise<Schichteinteilung[]> {
    ...
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Schichteinteilung[];
    return hydrateRecords(records, 'schichteinteilung');
  }
  static async getSchichteinteilungEntry(id: string): Promise<Schichteinteilung | undefined> {
    ...
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Schichteinteilung;
    return hydrateRecords([record], 'schichteinteilung')[0];
  }
  static async createSchichteinteilungEntry(fields: CreateSchichteinteilung): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.SCHICHTEINTEILUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'schichteinteilung') });
    return { ...data, record_id: data.id };
  }
  static async updateSchichteinteilungEntry(id: string, fields: Partial<CreateSchichteinteilung>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.SCHICHTEINTEILUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'schichteinteilung') });
    return { ...data, record_id: data.id };
  }
  ...
  static async getSchichtartenverwaltung(): Promise<Schichtartenverwaltung[]> {
    ...
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Schichtartenverwaltung[];
    return hydrateRecords(records, 'schichtartenverwaltung');
  }
  static async getSchichtartenverwaltungEntry(id: string): Promise<Schichtartenverwaltung | undefined> {
    ...
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Schichtartenverwaltung;
    return hydrateRecords([record], 'schichtartenverwaltung')[0];
  }
  static async createSchichtartenverwaltungEntry(fields: CreateSchichtartenverwaltung): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.SCHICHTARTENVERWALTUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'schichtartenverwaltung') });
    return { ...data, record_id: data.id };
  }
  static async updateSchichtartenverwaltungEntry(id: string, fields: Partial<CreateSchichtartenverwaltung>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.SCHICHTARTENVERWALTUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'schichtartenverwaltung') });
    return { ...data, record_id: data.id };
  }
