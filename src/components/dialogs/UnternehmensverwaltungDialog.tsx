/**
 * UnternehmensverwaltungDialog — pre-generated create/edit dialog for Unternehmensverwaltung.
 *
 ...
 * UnternehmensverwaltungDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Unternehmensverwaltung['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<UnternehmensverwaltungDialogDefaults | undefined>(undefined)
 */
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Unternehmensverwaltung';
...
/** Widened prefill type for UnternehmensverwaltungDialog.defaultValues — see file header. */
export type UnternehmensverwaltungDialogDefaults = Unternehmensverwaltung['fields'];
...
interface UnternehmensverwaltungDialogProps {
  ...
  defaultValues?: UnternehmensverwaltungDialogDefaults;
...
export function UnternehmensverwaltungDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: UnternehmensverwaltungDialogProps) {
  ...
  const REQUIRED_FIELDS = ['unternehmen_name', 'unternehmen_plz', 'unternehmen_ort', 'unternehmen_strasse', 'unternehmen_hausnummer'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Unternehmensverwaltung['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'unternehmensverwaltung');
      await onSubmit(clean as Unternehmensverwaltung['fields']);
      onClose();
    } catch (err) {
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('unternehmensverwaltung') })
    : t('new_entity', { entity: appLabel('unternehmensverwaltung') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'unternehmen_name': (
      <div key="unternehmen_name" className="space-y-1.5">
        <Label htmlFor="unternehmen_name">{fieldLabel('unternehmensverwaltung', 'unternehmen_name')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="unternehmen_name"
          placeholder=""
          ...
          required
        />
        {showErrors && !fields.unternehmen_name && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'unternehmen_plz': (
      <div key="unternehmen_plz" className="space-y-1.5">
        <Label htmlFor="unternehmen_plz">{fieldLabel('unternehmensverwaltung', 'unternehmen_plz')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="unternehmen_plz"
          placeholder=""
          ...
          required
        />
        {showErrors && !fields.unternehmen_plz && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'unternehmen_notiz': (
      <div key="unternehmen_notiz" className="space-y-1.5">
        <Label htmlFor="unternehmen_notiz">{fieldLabel('unternehmensverwaltung', 'unternehmen_notiz')}</Label>
        <Textarea
    ...
    'unternehmen_ort': (
      <div key="unternehmen_ort" className="space-y-1.5">
        <Label htmlFor="unternehmen_ort">{fieldLabel('unternehmensverwaltung', 'unternehmen_ort')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="unternehmen_ort"
          placeholder=""
          ...
          required
        />
        {showErrors && !fields.unternehmen_ort && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'unternehmen_strasse': (
      <div key="unternehmen_strasse" className="space-y-1.5">
        <Label htmlFor="unternehmen_strasse">{fieldLabel('unternehmensverwaltung', 'unternehmen_strasse')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          ...
          required
        />
        {showErrors && !fields.unternehmen_strasse && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'unternehmen_hausnummer': (
      <div key="unternehmen_hausnummer" className="space-y-1.5">
        <Label htmlFor="unternehmen_hausnummer">{fieldLabel('unternehmensverwaltung', 'unternehmen_hausnummer')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          ...
          required
        />
        {showErrors && !fields.unternehmen_hausnummer && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
  ...
  const FIELD_LABELS: Record<string, string> = {"unternehmen_name": "Name des Unternehmens", "unternehmen_plz": "Postleitzahl", "unternehmen_notiz": "Notiz (optional)", "unternehmen_ort": "Ort", "unternehmen_strasse": "Straße", "unternehmen_hausnummer": "Hausnummer"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.UNTERNEHMENSVERWALTUNG} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
