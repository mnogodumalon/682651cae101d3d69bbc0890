...
export type MitarbeiterverwaltungDialogDefaults = Mitarbeiterverwaltung['fields'];
...
export function MitarbeiterverwaltungDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: MitarbeiterverwaltungDialogProps) {
  ...
  const REQUIRED_FIELDS = ['mitarbeiter_vorname', 'mitarbeiter_nachname', 'mitarbeiter_email'] as const;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'mitarbeiter_vorname': (
      <div key="mitarbeiter_vorname" className="space-y-1.5">
        <Label htmlFor="mitarbeiter_vorname">{fieldLabel('mitarbeiterverwaltung', 'mitarbeiter_vorname')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          ...
          required
        />
        {showErrors && !fields.mitarbeiter_vorname && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'mitarbeiter_telefon': (
      <div key="mitarbeiter_telefon" className="space-y-1.5">
        <Label htmlFor="mitarbeiter_telefon">{fieldLabel('mitarbeiterverwaltung', 'mitarbeiter_telefon')}</Label>
        <Input
    ...
    'mitarbeiter_nachname': (
      <div key="mitarbeiter_nachname" className="space-y-1.5">
        <Label htmlFor="mitarbeiter_nachname">{fieldLabel('mitarbeiterverwaltung', 'mitarbeiter_nachname')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          ...
          required
        />
        {showErrors && !fields.mitarbeiter_nachname && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'mitarbeiter_email': (
      <div key="mitarbeiter_email" className="space-y-1.5">
        <Label htmlFor="mitarbeiter_email">{fieldLabel('mitarbeiterverwaltung', 'mitarbeiter_email')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
        ...
        />
        {showErrors && !fields.mitarbeiter_email && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
  ...
  const FIELD_LABELS: Record<string, string> = {"mitarbeiter_vorname": "Vorname", "mitarbeiter_telefon": "Telefonnummer", "mitarbeiter_nachname": "Nachname", "mitarbeiter_email": "E-Mail-Adresse"};
