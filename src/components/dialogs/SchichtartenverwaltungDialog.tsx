/**
 * SchichtartenverwaltungDialog — pre-generated create/edit dialog for Schichtartenverwaltung.
 *
 ...
 * SchichtartenverwaltungDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Schichtartenverwaltung['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<SchichtartenverwaltungDialogDefaults | undefined>(undefined)
 */
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Schichtartenverwaltung';
...
/** Widened prefill type for SchichtartenverwaltungDialog.defaultValues — see file header. */
export type SchichtartenverwaltungDialogDefaults = Schichtartenverwaltung['fields'];
...
interface SchichtartenverwaltungDialogProps {
  ...
  defaultValues?: SchichtartenverwaltungDialogDefaults;
...
export function SchichtartenverwaltungDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: SchichtartenverwaltungDialogProps) {
  ...
  const REQUIRED_FIELDS = ['schichtart_name', 'schichtart_ende', 'schichtart_beginn'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Schichtartenverwaltung['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'schichtartenverwaltung');
      await onSubmit(clean as Schichtartenverwaltung['fields']);
      onClose();
    } catch (err) {
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('schichtartenverwaltung') })
    : t('new_entity', { entity: appLabel('schichtartenverwaltung') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'schichtart_name': (
      <div key="schichtart_name" className="space-y-1.5">
        <Label htmlFor="schichtart_name">{fieldLabel('schichtartenverwaltung', 'schichtart_name')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="schichtart_name"
          placeholder=""
          ...
          required
        />
        {showErrors && !fields.schichtart_name && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'schichtart_beschreibung': (
      <div key="schichtart_beschreibung" className="space-y-1.5">
        <Label htmlFor="schichtart_beschreibung">{fieldLabel('schichtartenverwaltung', 'schichtart_beschreibung')}</Label>
        <Textarea
    ...
    'schichtart_ende': (
      <div key="schichtart_ende" className="space-y-1.5">
        <Label htmlFor="schichtart_ende">{fieldLabel('schichtartenverwaltung', 'schichtart_ende')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="schichtart_ende"
          placeholder=""
          ...
          required
        />
        {showErrors && !fields.schichtart_ende && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'schichtart_beginn': (
      <div key="schichtart_beginn" className="space-y-1.5">
        <Label htmlFor="schichtart_beginn">{fieldLabel('schichtartenverwaltung', 'schichtart_beginn')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          ...
          required
        />
        {showErrors && !fields.schichtart_beginn && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
  ...
  const FIELD_LABELS: Record<string, string> = {"schichtart_name": "Name der Schichtart", "schichtart_beschreibung": "Beschreibung", "schichtart_ende": "Ende (Uhrzeit)", "schichtart_beginn": "Beginn (Uhrzeit)"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.SCHICHTARTENVERWALTUNG} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
