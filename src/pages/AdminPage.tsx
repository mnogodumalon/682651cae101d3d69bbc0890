...
import { UnternehmensverwaltungViewDialog } from '@/components/dialogs/UnternehmensverwaltungViewDialog';
import { SchichteinteilungDialog } from '@/components/dialogs/SchichteinteilungDialog';
import { SchichteinteilungViewDialog } from '@/components/dialogs/SchichteinteilungViewDialog';
import { SchichtartenverwaltungDialog } from '@/components/dialogs/SchichtartenverwaltungDialog';
import { SchichtartenverwaltungViewDialog } from '@/components/dialogs/SchichtartenverwaltungViewDialog';
...
const UNTERNEHMENSVERWALTUNG_FIELDS = [
  { key: 'unternehmen_name', label: 'Name des Unternehmens', type: 'string/text' },
  { key: 'unternehmen_plz', label: 'Postleitzahl', type: 'string/text' },
  { key: 'unternehmen_notiz', label: 'Notiz (optional)', type: 'string/textarea' },
  { key: 'unternehmen_ort', label: 'Ort', type: 'string/text' },
  { key: 'unternehmen_strasse', label: 'Straße', type: 'string/text' },
  { key: 'unternehmen_hausnummer', label: 'Hausnummer', type: 'string/text' },
];
const SCHICHTEINTEILUNG_FIELDS = [
  { key: 'zuweisung_notiz', label: 'Notiz (optional)', type: 'string/textarea' },
  { key: 'zuweisung_ende', label: 'Ende (Uhrzeit)', type: 'string/text' },
  { key: 'zuweisung_datum', label: 'Datum der Schicht', type: 'date/date' },
  { key: 'zuweisung_unternehmen', label: 'Unternehmen auswählen', type: 'applookup/select', targetEntity: 'unternehmensverwaltung', targetAppId: 'UNTERNEHMENSVERWALTUNG', displayField: 'unternehmen_name' },
  { key: 'zuweisung_schichtart', label: 'Schichtart auswählen', type: 'applookup/select', targetEntity: 'schichtartenverwaltung', targetAppId: 'SCHICHTARTENVERWALTUNG', displayField: 'schichtart_name' },
  { key: 'zuweisung_beginn', label: 'Beginn (Uhrzeit)', type: 'string/text' },
  { key: 'zuweisung_mitarbeiter', label: 'Mitarbeiter auswählen', type: 'applookup/select', targetEntity: 'mitarbeiterverwaltung', targetAppId: 'MITARBEITERVERWALTUNG', displayField: 'mitarbeiter_vorname' },
];
const SCHICHTARTENVERWALTUNG_FIELDS = [
  { key: 'schichtart_name', label: 'Name der Schichtart', type: 'string/text' },
  { key: 'schichtart_beschreibung', label: 'Beschreibung', type: 'string/textarea' },
  { key: 'schichtart_ende', label: 'Ende (Uhrzeit)', type: 'string/text' },
  { key: 'schichtart_beginn', label: 'Beginn (Uhrzeit)', type: 'string/text' },
];
const MITARBEITERVERWALTUNG_FIELDS = [
  { key: 'mitarbeiter_vorname', label: 'Vorname', type: 'string/text' },
  { key: 'mitarbeiter_telefon', label: 'Telefonnummer', type: 'string/tel' },
  { key: 'mitarbeiter_nachname', label: 'Nachname', type: 'string/text' },
  { key: 'mitarbeiter_email', label: 'E-Mail-Adresse', type: 'string/email' },
];

const ENTITY_TABS = [
  { key: 'unternehmensverwaltung', pascal: 'Unternehmensverwaltung' },
  { key: 'schichteinteilung', pascal: 'Schichteinteilung' },
  { key: 'schichtartenverwaltung', pascal: 'Schichtartenverwaltung' },
  { key: 'mitarbeiterverwaltung', pascal: 'Mitarbeiterverwaltung' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('unternehmensverwaltung');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'unternehmensverwaltung': new Set(),
    'schichteinteilung': new Set(),
    'schichtartenverwaltung': new Set(),
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'unternehmensverwaltung': return (data as any).unternehmensverwaltung as Unternehmensverwaltung[] ?? [];
      case 'schichteinteilung': return (data as any).schichteinteilung as Schichteinteilung[] ?? [];
      case 'schichtartenverwaltung': return (data as any).schichtartenverwaltung as Schichtartenverwaltung[] ?? [];
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'schichteinteilung':
        lists.unternehmensverwaltungList = (data as any).unternehmensverwaltung ?? [];
        lists.schichtartenverwaltungList = (data as any).schichtartenverwaltung ?? [];
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'schichteinteilung' && fieldKey === 'zuweisung_unternehmen') {
      const match = (lists.unternehmensverwaltungList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.unternehmen_name ?? '—';
    }
    if (entity === 'schichteinteilung' && fieldKey === 'zuweisung_schichtart') {
      const match = (lists.schichtartenverwaltungList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.schichtart_name ?? '—';
    }
    if (entity === 'schichteinteilung' && fieldKey === 'zuweisung_mitarbeiter') {
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    const raw: any[] = (() => {
      switch (entity) {
        case 'unternehmensverwaltung': return UNTERNEHMENSVERWALTUNG_FIELDS as any[];
        case 'schichteinteilung': return SCHICHTEINTEILUNG_FIELDS as any[];
        case 'schichtartenverwaltung': return SCHICHTARTENVERWALTUNG_FIELDS as any[];
        case 'mitarbeiterverwaltung': return MITARBEITERVERWALTUNG_FIELDS as any[];
        default: return [];
      }
    })();
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'unternehmensverwaltung': return {
        create: (fields: any) => LivingAppsService.createUnternehmensverwaltungEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateUnternehmensverwaltungEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteUnternehmensverwaltungEntry(id),
      };
      case 'schichteinteilung': return {
        create: (fields: any) => LivingAppsService.createSchichteinteilungEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateSchichteinteilungEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteSchichteinteilungEntry(id),
      };
      case 'schichtartenverwaltung': return {
        create: (fields: any) => LivingAppsService.createSchichtartenverwaltungEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateSchichtartenverwaltungEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteSchichtartenverwaltungEntry(id),
      };
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'unternehmensverwaltung' || dialogState?.entity === 'unternehmensverwaltung') && (
        <UnternehmensverwaltungDialog
          open={createEntity === 'unternehmensverwaltung' || dialogState?.entity === 'unternehmensverwaltung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'unternehmensverwaltung' ? handleUpdate : (fields: any) => handleCreate('unternehmensverwaltung', fields)}
          defaultValues={dialogState?.entity === 'unternehmensverwaltung' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Unternehmensverwaltung']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Unternehmensverwaltung']}
        />
      )}
      {(createEntity === 'schichteinteilung' || dialogState?.entity === 'schichteinteilung') && (
        <SchichteinteilungDialog
          open={createEntity === 'schichteinteilung' || dialogState?.entity === 'schichteinteilung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'schichteinteilung' ? handleUpdate : (fields: any) => handleCreate('schichteinteilung', fields)}
          defaultValues={dialogState?.entity === 'schichteinteilung' ? dialogState.record?.fields : undefined}
          unternehmensverwaltungList={(data as any).unternehmensverwaltung ?? []}
          schichtartenverwaltungList={(data as any).schichtartenverwaltung ?? []}
      ...
      {(createEntity === 'schichtartenverwaltung' || dialogState?.entity === 'schichtartenverwaltung') && (
        <SchichtartenverwaltungDialog
          open={createEntity === 'schichtartenverwaltung' || dialogState?.entity === 'schichtartenverwaltung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'schichtartenverwaltung' ? handleUpdate : (fields: any) => handleCreate('schichtartenverwaltung', fields)}
          defaultValues={dialogState?.entity === 'schichtartenverwaltung' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'unternehmensverwaltung' && (
        <UnternehmensverwaltungViewDialog
          open={viewState?.entity === 'unternehmensverwaltung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'unternehmensverwaltung', record: r }); }}
        />
      )}
      {viewState?.entity === 'schichteinteilung' && (
        <SchichteinteilungViewDialog
          open={viewState?.entity === 'schichteinteilung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'schichteinteilung', record: r }); }}
          unternehmensverwaltungList={(data as any).unternehmensverwaltung ?? []}
          schichtartenverwaltungList={(data as any).schichtartenverwaltung ?? []}
          mitarbeiterverwaltungList={(data as any).mitarbeiterverwaltung ?? []}
        />
      )}
      {viewState?.entity === 'schichtartenverwaltung' && (
        <SchichtartenverwaltungViewDialog
          open={viewState?.entity === 'schichtartenverwaltung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'schichtartenverwaltung', record: r }); }}
        />
      )}
